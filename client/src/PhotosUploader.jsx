...
export default function PhotosUploader({addedPhotos,onChange}){
    ...
    async function addPhotoByLink(ev) {
        ...
        // });
        setPhotoLink(''); //After uploading photoes we will reset the state
      }
    
      function uploadPhoto(ev) {
        const files = ev.target.files;//files-> are the file which is uploaded
        // console.log(files);
    ...
    return(
        <>
            ...
            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {/* If we have add existing photoes */}
                {addedPhotos.length > 0 && addedPhotos.map(link=>(
                  <div className="h-32 flex relative" key={link}> {/*key={link}*/}
                    {/* {link} */}
                    <img className="rounded-2xl w-full object-cover" src={getPhotoUrl(link)} alt=""/>
